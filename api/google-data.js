...
async function handleGsheets(req, res) {
  ...
  if (req.method === 'GET') {
    const sheet = /^[\w\s-]{1,100}$/.test(req.query?.sheet || '') ? req.query.sheet : 'Sheet1';
    const range = /^[A-Z]{1,3}\d{1,7}:[A-Z]{1,3}\d{1,7}$/.test(req.query?.range || '') ? req.query.range : 'A1:Z1000';
...
async function handleYoutube(req, res) {
  ...
  let data;
  if (resource === 'liked') {
    data = await ytFetch('/videos?part=snippet&myRating=liked&maxResults=50&fields=items(id,snippet(title,channelTitle,description))');
  } else if (resource === 'subscriptions') {
    data = await ytFetch('/subscriptions?part=snippet&mine=true&maxResults=50&order=relevance&fields=items(snippet(title,resourceId,description))');
  } else if (resource === 'playlists') {
    data = await ytFetch('/playlists?part=snippet&mine=true&maxResults=50&fields=items(id,snippet(title,description))');
  } else {
