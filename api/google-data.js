/**
 * FinAI — Google Data Consolidator (owner-only)
 * Consolidates gmail, gcal, gsheets, and youtube-account into one
 * serverless function to stay within the Vercel Hobby plan 12-function cap.
 *
 * Route via query param:  ?svc=gmail | gcal | gsheets | youtube
 *
 ...
 *           OWNER_GSHEETS_REFRESH_TOKEN, OWNER_YOUTUBE_REFRESH_TOKEN,
 *           OWNER_SHEETS_ID
 */
...
async function getAccessToken(refreshTokenEnvVar) {
  ...
  if (!res.ok) throw new Error(`Token refresh failed for ${refreshTokenEnvVar}`);
...
async function handleGmail(req, res) {
  ...
  const token   = await getAccessToken('OWNER_GMAIL_REFRESH_TOKEN');
...
async function handleGcal(req, res) {
  ...
  const token = await getAccessToken('OWNER_GCAL_REFRESH_TOKEN');
...
async function handleGsheets(req, res) {
  ...
  const token   = await getAccessToken('OWNER_GSHEETS_REFRESH_TOKEN');
  const headers = { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' };

  if (req.method === 'GET') {
    const sheet = (req.query?.sheet || 'Sheet1').slice(0, 100);
    const range = (req.query?.range || 'A1:Z1000').slice(0, 30);
...
async function ytFetch(path, token) {
  ...
  if (res.status === 401) {
    const newToken = await getAccessToken('OWNER_YOUTUBE_REFRESH_TOKEN');
...
async function handleYoutube(req, res) {
  ...
  const token    = await getAccessToken('OWNER_YOUTUBE_REFRESH_TOKEN');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(400).json({ error: 'svc must be one of: gmail | gcal | gsheets | youtube' });
  } catch (err) {
