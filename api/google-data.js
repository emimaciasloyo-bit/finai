 ...
 * Dispatches on ?svc=gmail|gcal|gsheets|youtube
 *
...
async function getGoogleToken(refreshTokenEnv) {
...
async function handleGmail(req, res) {
  ...
  const token = await getGoogleToken(process.env.OWNER_GMAIL_REFRESH_TOKEN);
...
async function handleGcal(req, res) {
  ...
  const token = await getGoogleToken(process.env.OWNER_GCAL_REFRESH_TOKEN);
...
async function handleGsheets(req, res) {
  ...
  const token = await getGoogleToken(process.env.OWNER_GSHEETS_REFRESH_TOKEN);
  ...
  if (req.method === 'POST') {
    ...
    const appendRes = await fetch(
      ...
      { method: 'POST', headers, body: JSON.stringify({ values }), signal: AbortSignal.timeout(15_000) }
    );
...
async function ytFetch(path, accessToken) {
  ...
  if (res.status === 401) {
    const newToken = await getGoogleToken(process.env.OWNER_YOUTUBE_REFRESH_TOKEN);
    const retry = await fetch(`${YT_BASE}${path}`, { headers: { Authorization: `Bearer ${newToken}` } });
...
async function handleYoutube(req, res) {
  ...
  const token = await getGoogleToken(process.env.OWNER_YOUTUBE_REFRESH_TOKEN);
  let data;
  if (resource === 'liked') {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(400).json({ error: 'Missing ?svc=gmail|gcal|gsheets|youtube' });
  } catch (err) {
