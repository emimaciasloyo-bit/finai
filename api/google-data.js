 ...
 * Consolidates gcal, gmail, gsheets, youtube-account into one function
 * so the Vercel Hobby plan's 12-function limit is not exceeded.
 *
...
// ── gsheets handler ───────────────────────────────────────────────────────
const SHEET_RE = /^[A-Za-z0-9_ ]{1,100}$/;
...
async function handleGsheets(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (!SHEET_RE.test(sheet)) return res.status(400).json({ error: 'Invalid sheet name' });
...
export default async function handler(req, res) {
  ...
  try {
    switch (svc) {
      ...
      default:        return res.status(400).json({ error: 'svc must be gcal | gmail | gsheets | youtube' });
    }
  } catch (err) {
