 ...
 * serverless function to stay within the Vercel Hobby plan's 12-function limit.
 *
...
export default async function handler(req, res) {
  ...
  const handler_map = { gmail: handleGmail, gcal: handleGcal, gsheets: handleGsheets, youtube: handleYoutube };
  const svcHandler = handler_map[svc];
  if (!svcHandler) return res.status(400).json({ error: 'svc must be gmail | gcal | gsheets | youtube' });

  try {
    return await svcHandler(req, res);
  } catch (err) {
