 ...
 * Consolidated dispatcher for Gmail, Google Calendar, Google Sheets,
 * and YouTube Account — keeping the Vercel function count at 12.
 *
...
const VALID_SHEET = /^[\w\s\-()]{1,100}$/;
const VALID_RANGE = /^[A-Z]{1,3}\d{0,7}:[A-Z]{1,3}\d{0,7}$|^[A-Z]{1,3}\d{0,7}$/;
...
async function handleGsheets(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (!VALID_SHEET.test(sheet)) return res.status(400).json({ error: 'Invalid sheet name' });
    if (!VALID_RANGE.test(range))  return res.status(400).json({ error: 'Invalid range' });
