 ...
 * Combines gmail, gcal, gsheets, and youtube-account into one
...
const RANGE_RE       = /^[A-Z]{1,3}[0-9]{1,7}:[A-Z]{1,3}[0-9]{1,7}$/;
...
async function handleGsheets(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const appendRes = await fetch(
      ...
      { method: 'POST', headers, body: JSON.stringify({ values }), signal: AbortSignal.timeout(TIMEOUT_MS) },
    );
...
export default async function handler(req, res) {
  ...
  try {
    switch (svc) {
      ...
      default:        return res.status(400).json({ error: 'svc must be: gmail | gcal | gsheets | youtube' });
    }
  } catch (err) {
