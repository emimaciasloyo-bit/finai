 ...
 * serverless function to stay within Vercel Hobby's 12-function cap.
 *
...
async function handleGsheets(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const sheet        = String(req.query?.sheet || 'Sheet1').replace(/[^A-Za-z0-9 _-]/g, '').slice(0, 50);
    ...
    if (req.method === 'POST') {
      ...
      const safeSheet    = String(sheet).replace(/[^A-Za-z0-9 _-]/g, '').slice(0, 50);
