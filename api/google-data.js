 ...
 * Consolidates gcal, gmail, gsheets, and youtube-account into one
 * serverless function to stay within Vercel Hobby's 12-function limit.
 *
 * Route via ?svc=gcal|gmail|gsheets|youtube
 *
 * Auth: requires valid owner session cookie (set by /api/owner-session).
 * Env vars: GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and per-service
 *   refresh token vars (OWNER_GCAL_REFRESH_TOKEN, OWNER_GMAIL_REFRESH_TOKEN,
 *   OWNER_GSHEETS_REFRESH_TOKEN + OWNER_SHEETS_ID, OWNER_YOUTUBE_REFRESH_TOKEN).
 */
...
async function handleGcal(req, res) {
  ...
  const in60d  = new Date(Date.now() + 60 * 86400 * 1000).toISOString();

  const params = new URLSearchParams({
    timeMin: now, timeMax: in60d,
...
async function handleGsheets(req, res) {
  ...
  if (req.method === 'GET') {
    const sheet = String(req.query?.sheet || 'Sheet1').slice(0, 100);
    const range = String(req.query?.range || 'A1:Z1000').slice(0, 30);
    if (!SHEET_NAME_RE.test(sheet)) return res.status(400).json({ error: 'Invalid sheet name' });
    if (!RANGE_RE.test(range))      return res.status(400).json({ error: 'Invalid range' });
  ...
  const sheetSafe = String(sheet).slice(0, 100);
  if (!SHEET_NAME_RE.test(sheetSafe)) return res.status(400).json({ error: 'Invalid sheet name' });

  const rangeEncoded = encodeURIComponent(`${sheetSafe}!A1`);
...
export default async function handler(req, res) {
  ...
  const svc = String(req.query?.svc || '').toLowerCase();
  ...
  try {
    switch (svc) {
      ...
      default:
        return res.status(400).json({ error: 'Unknown service. Use: gcal | gmail | gsheets | youtube' });
    }
  } catch (err) {
    console.error(`[google-data ?svc=${svc}]`, err.message);
    return res.status(502).json({ error: 'Google API error. Please try again.' });
  }
}
