/**
 * FinAI — Google Data Proxy (owner-only)  api/google-data.js
 *
 ...
 *
 * Route: /api/google-data?svc=gmail|gcal|gsheets|youtube
 *
...
const RANGE_A1_RE    = /^[A-Za-z]{1,3}[0-9]{0,7}:[A-Za-z]{1,3}[0-9]{0,7}$/;
...
async function handleGsheets(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const range = String(req.query?.range || 'A1:Z1000');
      if (!SHEET_NAME_RE.test(sheet)) return res.status(400).json({ error: 'Invalid sheet name' });
      if (!RANGE_A1_RE.test(range))   return res.status(400).json({ error: 'Invalid range' });
    ...
    if (req.method === 'POST') {
      ...
      const sheetStr = String(sheet);
      if (!SHEET_NAME_RE.test(sheetStr)) return res.status(400).json({ error: 'Invalid sheet name' });
      const rangeEncoded = encodeURIComponent(`${sheetStr}!A1`);
...
export default async function handler(req, res) {
  ...
  if (svc === 'gmail')   return handleGmail(req, res);
  if (svc === 'gcal')    return handleGcal(req, res);
  if (svc === 'gsheets') return handleGsheets(req, res);
  if (svc === 'youtube') return handleYoutube(req, res);
