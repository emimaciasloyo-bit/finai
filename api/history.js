 ...
 * SECURITY: rate limiting, symbol validation, security headers, timeout
 */
...
// OWASP: strict symbol whitelist — prevents SSRF via crafted symbols
...
export default async function handler(req, res) {
  ...
  const { allowed } = rateLimit(ip, IP_MAX, IP_WIN);
  ...
  if (!SYM_RE.test(raw)) {
    return res.status(400).json({ error: 'Invalid symbol — must be 1-20 uppercase letters/digits/hyphens/dots' });
  }

  // Validate range
  const tf = (req.query.range || '1M').toUpperCase();
  if (!ALLOWED_RANGES.has(tf)) {
